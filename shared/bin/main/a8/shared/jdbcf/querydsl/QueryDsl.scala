...
class QueryDsl[T, TableDsl, K](
  val tableMapper: TableMapper[T],
  val tableDsl: TableDsl
) {

  implicit def keyedTableMapper: KeyedTableMapper[T, K] = tableMapper.asInstanceOf[KeyedTableMapper[T,K]]

  def query(whereFn: TableDsl => QueryDsl.Condition): SelectQuery[T, TableDsl] =
    SelectQueryImpl(tableDsl, tableMapper, whereFn(tableDsl), Nil)

  def update(set: TableDsl => Iterable[UpdateQuery.Assignment[?]]): UpdateQuery[TableDsl] =
    UpdateQueryImpl(
      tableDsl = tableDsl,
      outerMapper = tableMapper,
      assignments = set(tableDsl),
      where = Condition.TRUE
    )

  def updateRow(row: T)(where: TableDsl => QueryDsl.Condition)(using conn: Conn): Option[T] = {
    val selectQuery = SelectQueryImpl(tableDsl, tableMapper, where(tableDsl), Nil)
    conn
