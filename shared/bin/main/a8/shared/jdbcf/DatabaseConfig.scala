...
import a8.shared.jdbcf.DatabaseConfig.{DatabaseId, Password, Seconds}
...
object DatabaseConfig extends MxDatabaseConfig {
  ...
  object Seconds:
    given json.JsonTypedCodec[Seconds, json.ast.JsNum] = json.JsonCodec.long.dimap[Seconds](v => Seconds(v.toInt), _.toLong)
    def apply(v: Int): Seconds = v

  extension (s: Seconds)
    def inMillis: Int = s * 1000

  val oneMinuteSeconds: Seconds = Seconds(60)
  val twentyFourHoursInSeconds: Seconds = Seconds(24*60*60)
...
case class DatabaseConfig(
  ...
  minIdle: Int = 1,
  maxPoolSize: Int = 50,
  maxLifeTimeInSeconds: Seconds = twentyFourHoursInSeconds,
  connectionTimeoutInSeconds: Seconds = oneMinuteSeconds,
  idleTimeoutInSeconds: Seconds = twentyFourHoursInSeconds,
