...
object Conn extends Logging {
  ...
  object impl {
    ...
    def makeResource(connFn: Ctx ?=> java.sql.Connection, mapperCache: MapperMaterializer, jdbcUrl: Uri, dialect: Dialect, escaper: Escaper, config: DatabaseConfig): Resource[Conn] = {
      val jdbcMetadata = JdbcMetadata.default
      val key = AttributeKey[Conn].withSuffix(config.id.value.toString.toUpperCase)
      Resource
